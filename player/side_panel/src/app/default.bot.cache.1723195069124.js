...
app.controller('BotCtrl', ['$scope', '$rootScope', 'api', 'storage', 'modal',
	...
	function($scope, $rootScope, api, storage, modal, $location, $state, $timeout, $anchorScroll, mgError) {
			...
			showMsg("", ques.question, "animated fadeInLeft", 0)
				.then(function() {
					...
					if (ques.options && ques.options.length > 0) {
						...
						showActionButton("mgPlayerJSProd_multi", "animated fadeInRight", optList, 200)
							.then(function(o) {
								var optionStr = "";
								if (o && o.length) {
		...
		$scope.addRateAnswer = function(question, ans) {
			var html = "<wmgPlayerJSProd_ class='mgPlayerJSProd_rating' style='font-size:20px;'>";

			for (var i = question.options[0]; i <= question.options[1]; i++) {
				var className = "mgPlayerJSProd_lbl-btn mgPlayerJSProd_stars-rate";
				if (i < parseInt(ans) + 1) {
					html += "<button class='" + className + "' id='mgPlayerJSProd_rate-" + i + "' aria-label='rate' >&#9733;</button>";
				} else {
					html += "<button class='" + className + "' id='mgPlayerJSProd_rate-" + i + "' aria-label='rate' >&#9734;</button>";
				}
			}
		...
		$scope.showBotStepQuestion = function(steps, step_id) {
			...
			if (botStep && botStep.step_settings && botStep.step_settings.automation &&
				...
				if (!GmCXt.isEmpty(botStep.step_settings.tutorialUploadUrl) &&
					...
					var tutInlineHtml = "<div class='mgPlayerJSProd_chat-tg-content-wrapper'>";
					...
					if (botStep && botStep.step_video > 0 && botStep.step_video_url) {
						tutInlineHtml = tutInlineHtml + "<div class='mgPlayerJSProd_chat-guide-info-wrapper mgPlayerJSProd_video-info-wrapper'>" +
						...
						tutInlineHtml = tutInlineHtml + "<div class='mgPlayerJSProd_chat-info-wrapper mgPlayerJSProd_img-info-wrapper'>" +
							"<img class='mgPlayerJSProd_chat-tg-image' src='" + botStep.image_url + GmCXt.getCdnSign() + "' /></div>";
					}

					tutInlineHtml = tutInlineHtml + "<div class='mgPlayerJSProd_chat-tg-step-content' > " + stepText + " </div>" +
						"</div>";
